...
def get_best_candidate(cur_sol):
    ...
    #     recency_list.appendleft([best_current_no, best_temp_no, tabu_cnt_max])
    # else:
    #recency_list.pop()
    recency_list.appendleft([best_current_no, best_temp_no, tabu_cnt_max])

    for i in range(len(recency_list)):
    # print(recency_list[i][2])
        recency_list[i][2] = recency_list[i][2] - 1
    
    new_list = collections.deque()
    for list in recency_list:
        if(list[2] == 0):  
            continue
            # print("here")
            # recency_list.remove(list)
        else:
            new_list.append(list)

    recency_list = deepcopy(new_list)
    #print(recency_list)
