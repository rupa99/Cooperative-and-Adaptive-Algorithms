...
#The Goal: Change the initial starting point 10 times 
...
for i in range(4):
    ...
    for j in range(5):
        ...
        sol = sol - 1
    layout.append(col)
#print("the new layout is = ", layout)
...
#generating 10 initial solutions that are different
for k in range (10):
    #print("initial starting point = ", k+1)
    layout=[]
    curnt_sol = random.sample(range(1,21), 20)
    count = 0
    for i in range(4):
        col = []
        for j in range(5):
    ...
    print("the starting layout cost is = ", calculate_cost(layout))
